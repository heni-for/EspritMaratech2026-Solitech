...
export async function registerRoutes(
  ...
  app.get("/api/trainings", requireAuth, async (req, res) => {
    ...
    const allTrainings = await storage.getTrainings();
    const trainingsById = new Map<any, any>();
    allTrainings.forEach((t: any) => trainingsById.set(t.id, t));
    const enriched = await Promise.all(
  ...
  // ─── Admin Chat (Groq) ───
  app.post("/api/chat", requireRole("admin"), async (req, res) => {
    ...
    const [students, trainings, users, attendance] = await Promise.all([
      storage.getStudents(),
    ...
    const trainingsById = new Map<any, any>();
    trainings.forEach((t: any) => trainingsById.set(t.id, t));

    const levelsById = new Map<any, any>();
    const sessionsById = new Map<any, any>();
    for (const t of trainings) {
      const levels = await storage.getLevelsByTraining(t.id);
      for (const lvl of levels) {
        levelsById.set(lvl.id, lvl);
        const sessions = await storage.getSessionsByLevel(lvl.id);
    ...
    const studentsById = new Map<any, any>();
    students.forEach((s: any) => studentsById.set(s.id, s));

    const studentsList = students.slice(0, 20).map((s: any) => ({
      id: s.id,
    ...
    const trainingsList = trainings.slice(0, 20).map((t: any) => ({
      id: t.id,
      name: t.name,
      status: t.status,
    }));

    const trainersList = trainers.slice(0, 20).map((t: any) => ({
      id: t.id,
      name: t.fullName,
      username: t.username,
    }));

    const attendanceList = attendance.slice(0, 30).map((record: any) => {
      const student = studentsById.get(record.studentId);
      const session = sessionsById.get(record.sessionId);
      const level = session ? levelsById.get(session.levelId) : null;
      const training = level ? trainingsById.get(level.trainingId) : null;
      const date = (record.markedAt || session?.date || "").toString().split("T")[0];
      return {
        id: record.id,
        student: student ? `${student.firstName} ${student.lastName}` : "Inconnu",
        training: training?.name || "Inconnue",
        date,
    ...
    const systemPrompt = [
      "Tu es un assistant admin pour une plateforme de gestion d'eleves et presences.",
      "Reponds en francais, de maniere claire et courte.",
      "Par defaut, donne uniquement un nombre (compte) quand on demande 'combien'.",
      "Ne liste des noms que si l'utilisateur le demande explicitement (ex: 'liste', 'nomme', 'details').",
      "Si la question depasse les donnees fournies, dis-le et propose une action ou une question de clarification.",
      "Voici un extrait des donnees disponibles :",
      `Eleves (extrait): ${JSON.stringify(studentsList)}`,
      `Formations (extrait): ${JSON.stringify(trainingsList)}`,
      `Encadrants (extrait): ${JSON.stringify(trainersList)}`,
      `Presences (extrait): ${JSON.stringify(attendanceList)}`,
    ].join("\n");
    ...
    if (!groqRes.ok) {
      ...
      return res.status(502).json({ message: errText || "Groq request failed" });
    }
  ...
  app.post("/api/stt", requireRole("admin"), async (req, res) => {
    ...
    if (!sttRes.ok) {
      const errText = await sttRes.text();
      return res.status(502).json({ message: errText || "STT request failed" });
    }
  ...
  // ─── Dashboard (Admin) ───
  app.get("/api/dashboard/attendance", requireRole("admin"), async (req, res) => {
    ...
    const studentsById = new Map<any, any>();
    allStudents.forEach((s: any) => studentsById.set(s.id, s));

    const getDateStr = (record: any) => {
      const session = sessionsById.get(record.sessionId);
      const raw = record.markedAt || session?.date || "";
    ...
    const data: { label: string; present: number; absent: number }[] = [];
    const absents: Array<{
      id: any;
      studentName: string;
      trainingName: string;
    ...
    const rangeFilter = (dateStr: string, from: Date, to: Date) => {
    ...
    if (period === "day") {
      ...
      const dayRecords = attendance.filter((r: any) => getDateStr(r) === dateStr);
      data.push({
        label: "Aujourd'hui",
        present: dayRecords.filter((r: any) => r.present).length,
        absent: dayRecords.filter((r: any) => !r.present).length,
      });
      dayRecords
        ...
        .forEach((record: any) => {
          ...
          absents.push({
            ...
            trainingName: training?.name || "Inconnu",
            date: dateStr,
          });
        });
      const totals = {
        present: dayRecords.filter((r: any) => r.present).length,
        absent: dayRecords.filter((r: any) => !r.present).length,
        rate: dayRecords.length > 0
          ? Math.round((dayRecords.filter((r: any) => r.present).length / dayRecords.length) * 100)
          : 0,
      };
      return res.json({ data, totals, absents });
    }
    ...
    if (period === "week" || period === "month") {
      ...
      const periodRecords = attendance.filter((r: any) => rangeFilter(getDateStr(r), from, to));
      periodRecords
        .filter((r: any) => !r.present)
        .slice(0, 50)
        .forEach((record: any) => {
          ...
          absents.push({
            ...
            studentName: student ? `${student.firstName} ${student.lastName}` : "Inconnu",
            trainingName: training?.name || "Inconnu",
            date: getDateStr(record),
          });
        });
      const totals = {
        present: periodRecords.filter((r: any) => r.present).length,
        absent: periodRecords.filter((r: any) => !r.present).length,
        rate: periodRecords.length > 0
          ? Math.round((periodRecords.filter((r: any) => r.present).length / periodRecords.length) * 100)
          : 0,
      };
      return res.json({ data, totals, absents });
    }
    ...
    const monthsBack =
      period === "quarter" ? 2 : period === "semester" ? 5 : period === "year" ? 11 : 11;
    const bucket = new Map<string, { present: number; absent: number }>();
    for (let i = monthsBack; i >= 0; i--) {
      const d = new Date(today.getFullYear(), today.getMonth() - i, 1);
      bucket.set(fmtMonth(d), { present: 0, absent: 0 });
    }
    ...
    for (const record of attendance) {
      ...
      if (!entry) continue;
      if (record.present) entry.present++;
      else entry.absent++;
    }

    for (const [month, counts] of bucket.entries()) {
      data.push({
        label: month,
  ...
  app.get("/api/dashboard/stats", requireRole("admin"), async (_req, res) => {
    ...
    const todayAbsences = todayRecords.filter((r) => !r.present).length;
    ...
    const trainingProgress = await Promise.all(
      allTrainings.map(async (t) => {
        ...
        const sessionsByLevel = await Promise.all(
          trainingLevels.map(async (level) => ({
            level,
            sessions: await storage.getSessionsByLevel(level.id),
          }))
        );

        for (const { sessions } of sessionsByLevel) {
          totalSessions += sessions.length;
          totalPossible += sessions.length;
          for (const session of sessions) {
            if (session?.date && session.date >= todayStr) {
              if (!nextSessionDate || session.date < nextSessionDate) {
        ...
        for (const enrollment of enrolled) {
          ...
          for (const { sessions } of sessionsByLevel) {
            let allPresent = sessions.length > 0;
            for (const session of sessions) {
              const records = await storage.getAttendanceBySession(session.id);
        ...
        const avgProgressRaw = totalPossible > 0 ? Math.round((totalAttended / totalPossible) * 100) : 0;
        const avgProgress = Math.min(100, Math.max(0, avgProgressRaw));
        return {
          trainingId: t.id,
          trainingName: t.name,
